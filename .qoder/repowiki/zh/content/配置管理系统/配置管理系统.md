...
- 完整配置模式定义：调度器、存储、日志、告警、恢复、脚本管理等子系统的参数说明与约束。
...
## 核心组件
- 数据类模式定义：通过多级数据类组织配置，顶层为 AppConfig，包含调度器、存储、日志、告警、恢复、脚本管理等子配置对象。
...
## 结论
PyCronGuard 的配置系统以数据类为核心，结合 YAML 文件与严格的验证机制，提供了类型安全、易于扩展且具备热重载能力的配置方案。通过合理的默认值与清晰的分层结构，用户可以快速上手并在生产环境中安全地进行动态调整。
