...
| 选项 | 说明 |
|------|------|
| `--config / -c <path>` | 指定配置文件路径，默认 `~/.pycronguard/config.yaml` |
| `--verbose / -v` | 启用详细输出 |

用法示例：

```bash
pycronguard -c /path/to/config.yaml task list
pycronguard -v start
```
...
创建 `~/.pycronguard/` 目录结构、生成默认配置文件、初始化 SQLite 数据库。无额外参数。
...
启动后自动写入 PID 文件到 `~/.pycronguard/pycronguard.pid`。
...
# 带依赖的任务
pycronguard task add -n send-report \
    -s scripts/send.py -t daily -S "09:30" \
    --depends-on daily-report

# 每 30 秒执行的高频任务
pycronguard task add -n heartbeat \
...
```bash
pycronguard task remove <name> [OPTIONS]
```
...
```bash
pycronguard task history <name> [OPTIONS]
```
...
```bash
pycronguard script add [OPTIONS]
```
...
```bash
pycronguard script list [OPTIONS]
```
...
```bash
pycronguard script remove <name> [OPTIONS]
```
...
配置文件路径：`~/.pycronguard/config.yaml`（`pycronguard init` 自动生成）
...
# 数据库存储配置
storage:
  db_path: "~/.pycronguard/data.db"  # SQLite 数据库路径
...
# 脚本管理配置
script:
  script_dir: "~/.pycronguard/scripts"              # 脚本仓库目录
  version_dir: "~/.pycronguard/script_versions"     # 脚本版本备份目录
...
**服务文件模板** (`deploy/pycronguard.service`)：
...
Type=simple
User=%USER%
ExecStart=%PYTHON% -m pycronguard start --foreground
...
# 3. 查看状态
sudo systemctl status pycronguard

# 4. 查看日志
journalctl -u pycronguard -f
```

### macOS launchd

**plist 文件模板** (`deploy/com.pycronguard.plist`)：
...
<dict>
    ...
    <string>%HOME%/.pycronguard/logs/launchd_stdout.log</string>

    <key>StandardErrorPath</key>
    <string>%HOME%/.pycronguard/logs/launchd_stderr.log</string>
...
# 3. 卸载服务
launchctl unload ~/Library/LaunchAgents/com.pycronguard.plist

# 4. 查看服务状态
launchctl list | grep pycronguard
```
...
| `SIGTERM` | 同 SIGINT，用于 `pycronguard stop` 或系统服务停止 |
...
├── pycronguard.pid        # 守护进程 PID 文件
├── logs/                  # 日志目录
│   ├── pycronguard.log    # 当前日志文件（JSON 格式）
│   ├── pycronguard.log.2025-01-01  # 按天轮转的历史日志
...
- `config.yaml`：由 `pycronguard init` 生成，支持运行时热重载
