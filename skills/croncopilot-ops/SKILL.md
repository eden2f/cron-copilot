---
name: croncopilot-ops
description: CronCopilot 定时任务管理系统的运维使用指南。包含安装部署、任务管理、脚本编写、节假日配置和监控告警。Use when users ask about managing scheduled tasks with CronCopilot, adding cron jobs, deploying CronCopilot as a system service, or configuring holiday-aware scheduling.
---
...
CronCopilot 是一个 Python 定时任务管理与监控系统，支持 cron 表达式、节假日感知、异常自愈、系统服务部署。
...
```bash
# 1. 注册脚本
croncopilot script add --path scripts/my_task.py --name my-task

# 2. 创建任务
croncopilot task add -n my-task -s scripts/my_task.py -t daily -S "09:00"

# 3. 测试执行
croncopilot task run my-task

# 4. 启动调度器
croncopilot start          # 前台运行
croncopilot start -d       # 守护进程模式
```
...
| 命令 | 说明 |
|------|------|
| `croncopilot init` | 初始化配置和目录 |
| `croncopilot start [-d]` | 启动调度器（`-d` 守护进程） |
| `croncopilot stop` | 停止守护进程 |
| `croncopilot status` | 查看运行状态 |
| `croncopilot health` | 系统健康检查 |
| `croncopilot task add` | 添加定时任务 |
| `croncopilot task list` | 列出任务 |
| `croncopilot task run <name>` | 立即执行任务 |
| `croncopilot task remove <name>` | 删除任务 |
| `croncopilot task history <name>` | 查看执行历史 |
| `croncopilot script add` | 注册脚本 |
| `croncopilot script list` | 列出脚本 |
| `croncopilot script info <name>` | 查看脚本详情 |
...
```bash
# 添加每 5 分钟执行的监控任务
croncopilot task add -n health-check \
    -s scripts/check.py -t interval -S "5m"

# 添加带依赖的任务（B 依赖 A 完成后才执行）
croncopilot task add -n task-b \
...
# 按分类过滤
croncopilot task list --category report

# 立即测试执行
croncopilot task run daily-report

# 查看最近执行统计
croncopilot task history daily-report --stats-only

# 查看最近 7 天、最多 50 条记录
croncopilot task history daily-report --days 7 --limit 50

# 强制删除任务（不需确认）
croncopilot task remove old-task --force

# 注册脚本（带元信息）
croncopilot script add --path scripts/report.py \
    --name report --author "ops-team" \
    --description "每日报表生成" --category report

# 查看脚本详情和版本历史
croncopilot script info report

# 注销脚本并删除文件
croncopilot script remove report --delete-file
