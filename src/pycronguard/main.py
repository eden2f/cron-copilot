...
from pycronguard.logging.logger import get_logger, setup_logging

logger = get_logger(__name__)

# Default base directory
_BASE_DIR = os.path.expanduser("~/.pycronguard")
...
def _init_components(config_path: Optional[str] = None) -> Dict[str, Any]:
    ...
    from pycronguard.monitor.alert import AlertManager
...
def start(ctx: click.Context, daemon: bool, foreground: bool) -> None:
    ...
    # Daemon mode
    if daemon:
        from pycronguard.deploy.daemon import DaemonManager
    ...
    click.echo(f"✓ PyCronGuard 正在运行 (PID: {os.getpid()})")
...
def stop(ctx: click.Context) -> None:
    ...
    if not dm.is_running():
        click.echo("PyCronGuard 未在运行")
        return

    pid = dm.get_pid()
    click.echo(f"正在停止 PyCronGuard (PID: {pid}) ...")
...
def task_add(
    ...
    from pycronguard.core.task import TaskConfig, parse_schedule, task_config_to_record
