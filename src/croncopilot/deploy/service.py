"""System service configuration generator for CronCopilot.
...
class ServiceGenerator:
    ...
    def __init__(
        ...
        self._install_path: str = install_path or os.path.expanduser("~/.croncopilot")
    ...
    def generate(self, output_dir: Optional[str] = None) -> str:
        ...
        if plat == "linux":
            output_path = os.path.join(output_dir, "croncopilot.service")
            return self.generate_systemd(output_path)
        elif plat == "macos":
            output_path = os.path.join(output_dir, "com.croncopilot.plist")
            return self.generate_launchd(output_path)
        else:
            output_path = os.path.join(output_dir, "croncopilot.bat")
    ...
    def generate_systemd(self, output_path: str) -> str:
        ...
        content = textwrap.dedent(f"""\
            ...
            Type=simple
            User={user}
            ExecStart={self._python_path} -m croncopilot start --foreground
    ...
    def generate_windows(self, output_path: str) -> str:
        ...
        content = textwrap.dedent(f"""\
            @echo off
            REM CronCopilot - Windows Startup Script
            REM Place this file in shell:startup to run at login

            title CronCopilot
            echo Starting CronCopilot ...
            "{self._python_path}" -m croncopilot start --foreground
            pause
        """)
    ...
    def install_service(self) -> bool:
        ...
        print("\n" + "=" * 60)
        print("CronCopilot 服务安装指南")
        print("=" * 60)

        if plat == "linux":
            print(f"\n已生成 systemd 服务文件: {output_path}")
            print("\n请执行以下命令安装服务:")
            print(f"  sudo cp {output_path} /etc/systemd/system/croncopilot.service")
            print("  sudo systemctl daemon-reload")
            print("  sudo systemctl enable croncopilot")
            print("  sudo systemctl start croncopilot")
            print("\n查看状态:")
            print("  sudo systemctl status croncopilot")
            print("  journalctl -u croncopilot -f")

        elif plat == "macos":
            plist_dest = os.path.expanduser("~/Library/LaunchAgents/com.croncopilot.plist")
